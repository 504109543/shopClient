...
import com.lo.shop.utils.JsonFormatTool;
...
public class BaseCallback<T> extends AjaxCallback<T>
{
        ...
        if (null == mHandler)
        {
            mHandler = new Handler(){
                ...
                public void handleMessage(Message msg) {
                    if (msg.what == 1)
                    {
                        BaseCallback.this.performThrottling();
                    }
                }
            };
        }
    ...
    public void callback()
    {
        ...
        if (bytes > 0)
        {
            try {
                synchronized (bandwidthThrottlingLock)
                {
                    BaseCallback.incrementBandwidthUsedInLastSecond(bytes);
                }
            }
            catch (Exception e)
            {
    ...
    public void performThrottling()
    {
        if (BaseCallback.isBandwidthThrottled())
        {
            BaseCallback.measureBandwidthUsage();
        }
